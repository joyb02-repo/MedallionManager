...
if not st.session_state["authenticated"]:
        ...
        async function executeLoginPipeline() {
            ...
            try {
                ...
                if(data.status === "success") {
                    // Forward variables back out cleanly via parameter query simulation injections
    ...
    # Catch returning verification parameters generated by the parent runtime location redirect
    query_params = st.query_params
    if "auth_passcode" in query_params:
        test_passcode = query_params["auth_passcode"]
        # Double check validity real-time before flipping security variables
        try:
            ...
            if chk.status_code == 200 and chk.json().get("status") == "success":
                ...
                st.query_params.clear() # Clear parameters to keep the URL address line scrubbed clean
                st.rerun()
        except:
            pass
...
# ====================================================================
    ...
    <script>
        ...
        function commitClaimToSheets() {
            ...
            const pingUrl = endpoint + "?action=mineMedallion&user_passcode=" + encodeURIComponent("__PASSCODE_RAW__") + "&item=" + encodeURIComponent(selectedItem);
