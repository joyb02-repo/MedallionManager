...
<script>
    ...
    // 🎲 The odds now live entirely on the server (Apps Script reads the
    // Probability column from the Medallions sheet fresh on every roll, and
    // excludes anything sold out). This function just asks the server what was
    // won and plays a cycling reveal animation that lands on that real answer -
    // it no longer picks the winner itself, since a client-side pick could be
    // read out of the page source or replayed directly against the API.
    async function openMiningModal() {
    ...
    // Purely visual: cycles through medallion art before landing on the item
    // the server already committed. The increment already happened server-side
    // during openMiningModal()'s fetch call, so this never writes anything.
    function runRevealAnimation(finalItem) {
        ...
        function cycle() {
            ...
            if (speed < 320) {
                ...
                img.src = assetLibrary[finalItem];
                itemTxt.innerText = finalItem.toUpperCase() + "!";
                wrapper.style.opacity = "1";
                claimBtn.innerText = "Claim Medallion";
                claimBtn.disabled = false;
                claimBtn.classList.add('visible');
    ...
    // The medallion was already saved to the sheet the moment the server
    // resolved the roll, so this just closes the modal and refreshes the
    // dashboard's data - there is no second network write here.
